...
class StatusManagementNodelet : public nodelet::Nodelet {
 ...
 private:
  ...
  std::unique_ptr<CurrentPoseGenerator> p_cur_pose_gen_;
...
void StatusManagementNodelet::Initialize() {
  ...
  p_cur_pose_gen_.reset(new CurrentPoseGenerator(params_.cur_pose_gen_params_,
                                                 p_pubs_->ndt_config_pub_,
                                                 p_pubs_->ndt_init_pose_pub_));
...
XYZRPY StatusManagementNodelet::InitializePose(const init_config &conf) {
  ...
  p_cur_pose_gen_->InitializePose(init_pose);
...
void StatusManagementNodelet::CreateAndPublishCurrentPose(
    const ros::Time &time) {
  {
    ...
    if (p_cur_pose_gen_->GetCurrentPoseAndTransform(cur_pose.pose,
                                                    tf_odom_to_world)) {
...
void StatusManagementNodelet::RunPoseInitializerIfNecessary() {
  ...
  if (last_pose_init_time.is_zero() ||
    ...
    if (params_.localization_unreliable_cnt_for_reinit <=
      ...
      // X. Restart
      p_cur_pose_gen_->InitializePose(searched_pose);
