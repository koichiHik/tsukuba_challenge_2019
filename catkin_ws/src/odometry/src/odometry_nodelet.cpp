...
private:
  ...
  void parseJointStatesMsg(const sensor_msgs::JointStateConstPtr &jointPos, double &rRad, double &lRad, double &rVel,
                           double &lVel);
...
private:
  ...
  // ROS Sync Policy
  typedef message_filters::sync_policies::ApproximateTime<kmsgs::Imu, sensor_msgs::JointState> OdomSyncPolicy;
...
void OdometryNodelet::onInit()
{
  ...
  subWhl = new message_filters::Subscriber<sensor_msgs::JointState>(nh, jointStatesTopicName, 100);
...
void OdometryNodelet::readParams(OdomParams &odomParams)
{
  ...
  readParam("odom_imu_topic_name", nh_ns.param<std::string>("odom_imu_topic_name", odomImuTopicName, ""));

  // Frame Name
  readParam("odometry_imu_frame_name", nh_ns.param<std::string>("odometry_imu_frame_name", odomImuFrameName, ""));
  readParam("odometry_whl_frame_name", nh_ns.param<std::string>("odometry_whl_frame_name", odomWhlFrameName, ""));
  readParam("base_link_frame_name", nh_ns.param<std::string>("base_link_frame_name", baselinkFrameName, ""));

  // Hardware Related
  readParam("gyro_1bit_in_rad", nh_ns.param<double>("gyro_1bit_in_rad", odomParams.gyro1BitInRad, 0.0));
  ...
  // Odometry Parameter
  readParam("standstill_thresh", nh_ns.param<double>("standstill_thresh", odomParams.standstillThresh, 0.0));
  ...
  readParam("odom_whl_only", nh_ns.param<bool>("odom_whl_only", odom_whl_only, false));
...
void OdometryNodelet::parseJointStatesMsg(const sensor_msgs::JointStateConstPtr &jointPos, double &rRad, double &lRad,
...
void OdometryNodelet::fillOdomMessage(nav_msgs::Odometry &odomMsg, std::string odomFrameName, const ros::Time &stamp,
                                      const OdomPos &odomPos, const OdomVel &odomVel)
{
  // Header
